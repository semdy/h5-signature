...
class Stage {
  ...
  getResult(origin) {
    try {
      ...
      const { cutWidth, cutHeight, lOffset, tOffset } =
        this.getValidBound(canvas);
      ...
      const { exportMaxWidth, exportMaxHeight, exportPadding, scaleRatio } =
        this.options;
      ...
      if (exportMaxWidth || exportMaxHeight || exportPadding !== 0) {
        ...
        exportCtx.drawImage(
          ...
          exportPadding,
          exportPadding,
          exportCanvas.width - exportPadding * 2,
          exportCanvas.height - exportPadding * 2
        );
